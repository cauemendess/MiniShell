...
/*   Updated: 2024/06/12 18:15:22 by dfrade           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	exit_shell(char **argv)
{
	...
	{
		if (is_valid_number(argv[1]) == 1)
			number = ft_atoi(argv[1]);
		else
			print_exit_error("numeric argument required", NULL);
	}
	else if (argv[0] != NULL && argv[1] != NULL && argv[2] != NULL)
		print_exit_error("too many arguments", argv[2]);
...
void	print_exit_error(char *msg, char **argv)
{
	...
	message = malloc((ft_strlen(msg) + ft_strlen(argv) + 1) * sizeof(char));
	if (message == NULL)
		return ;
